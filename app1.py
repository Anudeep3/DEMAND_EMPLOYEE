...
def clean_employee(df):
    cols = ["EMPID","Employee Name","ONSITE_OFFSHORE","BAND","Grade",
    ...
    df["EMPID_str"] = df["EMPID"].astype(str).str.strip()
    return df
...
def match_demand_employee(df_demand, df_employee):
    ...
    res = (
        merged.groupby(["Request profile number","EMPID"], as_index=False)
        .agg(
    ...
    res["EMPID_str"] = res["EMPID"].astype(str).str.strip()
    return res
...
if demand_file and employee_file:
    ...
    df_matches_full=df_matches.merge(df_demand,on="Request profile number",how="left") \
                              .merge(df_employee,on="EMPID",how="left")
    ...
    df_final=df_starvation.merge(df_demand,left_on="Demand",right_on="Request profile number",how="left") \
                          .merge(df_employee,left_on="Assigned_Employee_key",right_on="EMPID_str",how="left")
